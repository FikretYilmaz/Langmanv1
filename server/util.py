...
def get_config(env, config_resource):
    ...
    Sets up the application config as follows:
        1. Sets the environment key from app.config['ENV'],
            which is automatically set by Flask.
        2. Loads config values from within the environment key of
            the config.yaml file
        3. Loads an FLASK_KEY=VAL environment variables as
            app.config['KEY']='VAL'
        4. For any variable with value starting with "env:"
    ...
    try:
        ...
        raise KeyError("Invalid ENV value '{}' should be in {}, set using \
            FLASK_ENV=value".format(
