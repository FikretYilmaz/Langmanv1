from sqlalchemy import Column, types, MetaData, ForeignKey
...
class Usage(base_usage):
    ...
    *``usage`` - Usage sentence, length <= 400, with world blanked
    *``source`` - Text from which the usage sentence is drawn
    '''
    __tablename__ = 'usage'
    usage_id = Column(types.Integer, primary_key=True)
    language = Column(types.Enum("en", "es", "fr", name='language_code'),
                      nullable=False)
...
class User(base_games):
    ...
    games = relationship('Game', foreign_keys='Game.player')  # ! Attention
    # ! it could be Game.player
    ...
    def _game_ended(self, outcome, time_delta):
        ...
        self.total_time = time_delta + (self.total_time or
...
class Game(base_games):
    ...
    player = Column(types.String(length=38), ForeignKey(User.user_id),
                    nullable=False)
    usage_id = Column(types.Integer, nullable=False)
    # no foreignkey across DBs
